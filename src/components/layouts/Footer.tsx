    ...
    <footer className="mt-24 border-t border-slate-200 bg-white">
      <div className="site-container grid gap-10 py-12 md:grid-cols-[1.4fr_.8fr_1fr]">
        <div>
          ...
          <p className="max-w-md text-sm leading-7 text-slate-600">
            معرفی خدمات بیمه پارسیان و راهنمایی برای انتخاب بیمه متناسب با
            نیازهای شخصی، خانوادگی و کسب‌وکار.
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="grid gap-3 text-sm text-slate-600">
            {insuranceProducts.slice(0, 5).map((item) => (
              <Link key={item.slug} href={`/insurance/${item.slug}`}>
                {item.shortTitle}
              </Link>
            ))}
          </div>
      ...
      <div className="border-t border-slate-100">
        <div className="site-container py-5 text-xs leading-6 text-slate-500">
          © {new Date().getFullYear()} نمایندگی آفرین صناعی. اطلاعات این وب‌سایت
          جنبه معرفی خدمات دارد و جزئیات هر پوشش تابع شرایط رسمی بیمه‌نامه است.
        </div>
      </div>
    </footer>
  );
